...
class LeCartWoocommerce(LeCartWordpress):
	...
	def clear_target_products(self):
		...
		while self._check_product_exists():
			...
			all_collections_query = {
				'type': 'query',
				'query': "DELETE FROM `_DBPRF_posts` WHERE post_type IN('product','revision','attachment')"
			}
			clear_table = self.get_connector_data(self.get_connector_url('query'),
			                                      {'query': json.dumps(all_collections_query)})
			if (not clear_table) or (clear_table['result'] != 'success') or (not clear_table['data']):
			...
			clear_table = self.get_connector_data(self.get_connector_url('query'),
												  {'query': json.dumps(del_term_rela_query)})
	...
	def clear_target_customers(self):
		...
		self.log(del_user_query, "clear_users_query")
		clear_table = self.get_connector_data(self.get_connector_url('query'), {
			'query': json.dumps({
				'type': 'query', 'query': del_user_query
			})
		})
		self.log(clear_table, "clear_users_table_query")
		if (not clear_table) or (clear_table['result'] != 'success') or (not clear_table['data']):
			self.log("Clear data failed. Error: Could not empty customers ", 'clear')
		...
		self.log(del_user_meta_query,"clear_usermeta_query")
		clear_table = self.get_connector_data(self.get_connector_url('query'), {
			'query': json.dumps({
				'type': 'query', 'query': del_user_meta_query
			})
		})
		self.log(clear_table, "clear_usermeta_table_query")
		...
		self.log(del_customer_lookup_query,"clear_customer_lookup")
		clear_table = self.get_connector_data(self.get_connector_url('query'), {
				'query': json.dumps({
					'type': 'query', 'query': del_customer_lookup_query
				})
			})
		self.log(clear_table, "clear_customer_lookup_table_query")
		self._notice['target']['clear'] = next_clear
		return self._notice['target']['clear']
	...
	def clear_target_orders(self):
		...
		while self._check_order_exists():
			...
			self.log(all_collections_query, "all_collections_query")
			clear_table = self.get_connector_data(self.get_connector_url('query'),
			                                      {'query': json.dumps(all_collections_query)})
			self.log(clear_table, "clear_allcollections_table_query")
			if (not clear_table) or (clear_table['result'] != 'success'):
				self.log("Clear data failed. Error: Could not empty products", 'clear')
				continue
			# clear meta post(orders)
			...
			self.log(all_meta_query,"all_meta_query")
			clear_table = self.get_connector_data(self.get_connector_url('query'),
			                                      {'query': json.dumps(all_meta_query)})
			self.log(clear_table, "clear_allmeta_table_query")
			if (not clear_table) or (clear_table['result'] != 'success'):
				self.log("Clear data failed. Error: Could not empty products", 'clear')
				continue
			...
			self.log(order_items_query,"order_items_query")
			clear_table = self.get_connector_data(self.get_connector_url('query'),
												  {'query': json.dumps(order_items_query)})
			self.log(clear_table, "clear_items_table_query")

			if (not clear_table) or (clear_table['result'] != 'success'):
				self.log("Clear data failed. Error: Could not empty products", 'clear')
				continue

			order_itemmeta_query = {
				'type': 'query',
			...
			self.log(order_itemmeta_query,"order_itemmeta_query")
			clear_table = self.get_connector_data(self.get_connector_url('query'),
												  {'query': json.dumps(order_itemmeta_query)})
			self.log(clear_table, "clear_itemmeta_table_query")
			if (not clear_table) or (clear_table['result'] != 'success'):
				self.log("Clear data failed. Error: Could not empty products", 'clear')
				continue
	...
	def category_import(self, convert, category, categories_ext):
		...
		category_query = self.create_insert_query_connector('terms', data)
		category_id = self.import_category_data_connector(category_query, True, convert['id'])
	...
	def after_category_import(self, category_id, convert, category, categories_ext):
		...
		term_taxonomy_id = self.import_category_data_connector(term_taxonomy_query, True,convert['id'])
		self.insert_map(self.TYPE_CATEGORY, convert['id'], term_taxonomy_id, convert['code'])
		...
		self.import_category_data_connector(self.create_insert_query_connector("termmeta", meta_insert), True,
										   convert['id'])
		thumbnail_id = False
		if convert['thumb_image']['url'] or convert['thumb_image']['path']:
		...
		thumbnail_meta_insert = {
			'term_id': category_id,
			'meta_key': '_thumbnail_id',
			'meta_value': thumbnail_id
		}
		self.import_data_connector(self.create_insert_query_connector("termmeta", thumbnail_meta_insert),"category")
		...
		return response_success(term_taxonomy_id)
	...
	# dung de import vao bang term_relationships(hien dang loi), khong can doc phan manufacture va tax do khong co
	...
	def product_import(self, convert, product, products_ext):
		data = {
			...
			'post_date_gmt': convert['created_at'] if convert['created_at'] and '0000-00-00' not in convert[
				'created_at'] else get_current_time(),
			'post_content': convert['description'] if convert['description'] else "",
		...
		product_id = self.import_page_data_connector(product_query, True, convert['id'])
		...
		for term_relationship in products_ext['data']['products_to_categories']:
			if term_relationship['products_id'] == convert['id']:
				...
				category_id = self.get_map_field_by_src(self.TYPE_CATEGORY,term_relationship['categories_id'], convert['code'])
	...
	def after_product_import(self, product_id, convert, product, products_ext):
		...
		postMeta_data = {
			...
			'_width': convert['width'] if convert['width'] else '',
			'_height': convert['height'] if convert['height'] else '',
			'_manage_stock': 'yes' if convert['sku'] else 'no',
			'_backorders': 'no',
		...
		for meta_key, value in postMeta_data.items():
			...
			self.import_product_data_connector(self.create_insert_query_connector("postmeta", postmeta_data), True,
											   convert['id'])
		thumbnail_id = False
		if convert['thumb_image']['url'] or convert['thumb_image']['path']:
			image_process = self.process_image_before_import(convert['thumb_image']['url'],
		...
		if thumbnail_id:
			...
			self.import_product_data_connector(self.create_insert_query_connector("postmeta", thumbnail_meta_insert), True, convert['id'])
		...
		data={
			...
			'onsale':1 if convert['special_price']['price'] else 0,
		...
		product_lookup_query = self.create_insert_query_connector('wc_product_meta_lookup', data)
		product_id = self.import_product_data_connector(product_lookup_query, True, convert['id'])
		return response_success(product_id)
	...
	def customer_import(self, convert, customer, customers_ext):
		customer_registered =''
		for customer_info in customers_ext['data']['customers_info']:
			if customer_info['customers_info_id'] == convert['id']:
				customer_registered = customer_info['customers_info_date_account_created']
				self.log(customer_registered,"customer_registered")
		data={
		...
		customer_query = self.create_insert_query_connector('users', data)
	...
	def after_customer_import(self, customer_id, convert, customer, customers_ext):
		self.log(convert,"convert_json")
		self.log(customer,"customer_json")
		self.log(customers_ext,"customer_ext_json")
		country=''
		postcode=''
		city=''
		state=''
		...
		for customer_info in customers_ext['data']['customers_info']:
			if customer_info['customers_info_id'] == convert['id']:
				customer_registered = customer_info['customers_info_date_account_created']
				customer_last_active = customer_info['customers_info_date_of_last_logon']
				customer_last_update = customer_info['customers_info_date_account_last_modified']
		...
		usermeta_data={
			...
			'wc_last_active':customer_last_active,
			'wp_capabilities':'a:1:{s:8:"customer";b:1;}'
		}
		for key, value in usermeta_data.items():
			meta_insert = {
				'user_id': customer_id,
				'meta_key': key,
				'meta_value': value
			}
			self.import_customer_data_connector(self.create_insert_query_connector("usermeta", meta_insert), True, convert['id'])
		...
		if convert['address']:
			...
			user_meta = {
				...
				'billing_state': bill_data['state']['name'],
				'billing_postcode': bill_data['postcode'],
				'billing_email': convert['email'],
				'billing_phone': bill_data['telephone'],
				...
				'shipping_state': ship_data['state']['name'],
			...
			self.log(user_meta,"usermetaquery")
		...
		customer_lookup_data = {
			...
			'last_name':convert['last_name'],
			'email':convert['email'],
			'date_last_active':customer_last_active,
		...
		customer_lookup_query = self.create_insert_query_connector('wc_customer_lookup', customer_lookup_data)
		customer_id = self.import_customer_data_connector(customer_lookup_query, True, convert['id'])
		return response_success(customer_id)
	...
	def order_import(self, convert, order, orders_ext):
		...
		data = {
			...
			'ping_status': 'closed',
			'post_name': 'order',
			'post_modified': convert['updated_at'] if convert['updated_at'] and '0000-00-00' not in convert[
				'updated_at'] else get_current_time(),
			'post_modified_gmt': convert['updated_at'] if convert['updated_at'] and '0000-00-00' not in convert[
				'updated_at'] else get_current_time(),
			'post_parent': 0,
		...
		self.log(order_query, "order_query")
	...
	def after_order_import(self, order_id, convert, order, orders_ext):
		...
		postmeta_data = {
			...
			'_customer_user':convert['billing_address']['id'],
			...
			'_completed_date':convert['created_at'],
			'_order_total': convert['total']['amount'],
		}
		self.log(postmeta_data,"log")
		for meta_key, value in postmeta_data.items():
			order_postmeta_data = {
				'post_id': order_id,
				'meta_key': meta_key,
			...
			meta_query = self.create_insert_query_connector("postmeta", order_postmeta_data)
			all_queries.append(meta_query)
		...
		for item in order_items:
			...
			order_item_query = self.create_insert_query_connector("woocommerce_order_items", order_item_data)
			self.log(order_item_query,"woocommerce_order_items")
			order_item_id = self.import_data_connector(order_item_query, 'order')
			...
			for meta_key, meta_value in order_item_meta.items():
				...
				meta_query = self.create_insert_query_connector("woocommerce_order_itemmeta", meta_insert)
				self.log(meta_query, "woocommerce_query")
				all_queries.append(meta_query)
			for option in item['options']:
				meta_insert = {
					'order_item_id': order_item_id,
				...
				meta_query = self.create_insert_query_connector("woocommerce_order_itemmeta", meta_insert)
				self.log(meta_query,"options_query")
				all_queries.append(meta_query)
		if all_queries:
	...
	def insert_map(self, map_type = None, id_src = None, id_desc = None, code_src = None, code_desc = None, value = None, lang = None):
		...
		self.log(data_inset,"data_inset")
