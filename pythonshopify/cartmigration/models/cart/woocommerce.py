...
class LeCartWoocommerce(LeCartWordpress):
	...
	def before_0(self, convert, tax, taxes_ext):
		return response_success()
	...
	custom.get_customers_main_export
	...
	def after_review_import(self, review_id, convert, review, reviews_ext):
		...
		for rating in ratings:
			...
			for meta_key, meta_value in comment_meta.items():
				...
				self.import_data_connector(meta_query, 'user')
		return response_success()
			...
			if self.is_wpml():
				...
				if attribute_data_lang['option_value_name'] != attribute_value:
					translate_query = {
						'icl_translations': {
							'type': 'select',
							'query': 'select * from _DBPRF_icl_translations where trid in (select trid from _DBPRF_icl_translations where  ' + self.dict_to_where_condition({'element_id': opt_value_id, 'element_type': 'tax_pa_' + pro_attr_code}) + ')'
						},
						'term': {
							'type': 'select',
